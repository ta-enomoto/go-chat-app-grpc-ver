...
import (
	...
	chatapisvr "chat-api/gen/grpc/chatapi/server"
	"context"
	"log"
	"net"
	"net/url"
...
func handleGRPCServer(ctx context.Context, u *url.URL, chatapiEndpoints *chatapi.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	{
		chatapiServer = chatapisvr.New(chatapiEndpoints, nil)
	}
	...
	chatapipb.RegisterChatapiServer(srv, chatapiServer)
