...
func main() {
	...
	{
		logger = log.New(os.Stderr, "[getchat] ", log.Ltime)
	}

	// Initialize the services.
	var (
		chatapiSvc chatapi.Service
	)
	{
		chatapiSvc = getchat.NewChatapi(logger)
	}

	// Wrap the services in endpoints that can be invoked from other services
	// potentially running in different processes.
	var (
		chatapiEndpoints *chatapi.Endpoints
	)
	{
		chatapiEndpoints = chatapi.NewEndpoints(chatapiSvc)
	}
